...
def selecionar_cubo():
    ...
    red1 = float(input("Digite o valor mínimo [RED]: "))
    green = float(input("Digite o valor máximo [GREEN]: "))
    green1 = float(input("Digite o valor mínimo [GREEN]: "))
    blue = float(input("Digite o valor máximo [BLUE]: "))
    blue1 = float(input("Digite o valor mínimo [BLUE]: "))

    for i in range(altura):
        for j in range(largura):
            if ( (float(img[i,j,2]) < red) and 
                (float(img[i,j,2]) > red1) and 
                (float(img[i,j,1]) < green) and 
                (float(img[i,j,1]) > green1) and 
                (float(img[i,j,0]) < blue) and 
                (float(img[i,j,0]) > blue1) ):
                img[i,j,2] = 255
                img[i,j,1] = 255
                img[i,j,0] = 255
